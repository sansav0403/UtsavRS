...
- (void)viewDidLoad {
    ...
    mainFieldArray = [[NSMutableArray alloc] initWithObjects:SpaBookingFormService, SpaBookingFormDate, SpaBookingFormTime,nil];
    
    RSSpaLocation *selectedSpaLocation = [[RSSelectedSpaLocation sharedInstance] spaLocation];
    
    (selectedSpaLocation.allowGender)?([mainFieldArray addObject:SpaBookingFormStafGender]):nil;
    
    (selectedSpaLocation.allowStaff)?([mainFieldArray addObject:SpaBookingFormStaff]):nil;
    ...
    (selectedSpaLocation.allowGender)?([subFieldArray addObject:SpaBookingNoPref]):nil;
    (selectedSpaLocation.allowStaff)?([subFieldArray addObject:SpaBookingNoPref]):nil;
    ...
    CGRect checkAvailButtonFrame = ([UIManager isCurrentDeviceTypeIPhone5])?ButtonImageFrame568:ButtonImageFrame;
	checkAvailButton = [[UIButton alloc] initWithFrame:checkAvailButtonFrame];
    ...
    [checkAvailButton setTitle:kCheckAvailability_Title forState:UIControlStateNormal];
    
    [checkAvailButton setBackgroundImage:enabledCheckImage forState:UIControlStateNormal];
    [checkAvailButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [checkAvailButton.titleLabel setFont:[UIFont boldSystemFontOfSize:kLabelTextFontSize]];
    
    
    [checkAvailButton setBackgroundImage:disabledCheckImage forState:UIControlStateDisabled];
    [checkAvailButton setTitleColor:[UIColor grayColor] forState:UIControlStateDisabled];
    [checkAvailButton titleLabel].shadowOffset = CGSizeMake(0, -1);
...
- (void) onStaffPickerValueChanged:(NSNotification *)notice
{
    ...
    //Updated on 30-Oct
    if ([selectedOption isEqualToString:SpaBookingFormStafGender] && [subFieldArray count]>selectedSection+1) {
		[subFieldArray removeObjectAtIndex:selectedSection+1];
		[subFieldArray insertObject:SpaBookingNoPref atIndex:selectedSection+1];
	}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    {
        //Updated after change request made on 26-Oct-12
        selectedSection =  indexPath.row;
        selectedOption = [mainFieldArray objectAtIndex:indexPath.row];
        
        if ([selectedOption isEqualToString:SpaBookingFormService]) {
            DLog(@"Load Spa Services View...");
            //fetch spa service at the start and create dictionary
            [self fetchSpaServices];
        }
        else if([selectedOption isEqualToString:SpaBookingFormDate])
        {
        ...
        else if([selectedOption isEqualToString:SpaBookingFormTime])
        {
        ...
        else if([selectedOption isEqualToString:SpaBookingFormStafGender])
        {
            RSSpaStaffSeletionVC *spaStaffSelVC = [[RSSpaStaffSeletionVC alloc] initWithSection:kGenderkey prefStaffs:prefStaffGenders];
            [self.navigationController pushViewController:spaStaffSelVC animated:YES];
            [spaStaffSelVC release];
        }
        else if([selectedOption isEqualToString:SpaBookingFormStaff])
        {
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component
{
    ...
    if ([selectedOption isEqualToString:SpaBookingFormStafGender]) {
		return 3;
	}
	else if ([selectedOption isEqualToString:SpaBookingFormStaff]) {
		return [prefStaffNames count];
	  ...
	  inComponent:(NSInteger)component
{
    ...
    if ([selectedOption isEqualToString:SpaBookingFormStafGender]) {
        ...
        if ([subFieldArray count]>selectedSection+1) {
            [subFieldArray removeObjectAtIndex:selectedSection+1];
            [subFieldArray insertObject:SpaBookingNoPref atIndex:selectedSection+1];
        }
...
-(void) loadStaffNamesinPicker:(id) parserModelData
{
	...
	RSSpaStaffSeletionVC *spaStaffSelVC = [[RSSpaStaffSeletionVC alloc] initWithSection:kStaffkey
