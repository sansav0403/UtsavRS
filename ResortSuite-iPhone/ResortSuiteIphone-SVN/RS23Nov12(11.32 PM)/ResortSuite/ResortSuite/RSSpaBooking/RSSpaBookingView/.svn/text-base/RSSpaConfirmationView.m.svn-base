...
- (void)viewDidLoad
{
    ...
    //self.navigationItem.title = [NSString stringWithFormat:@"%@ %@",kBook_Title,location.spaLocation.locationName];
    NSString *viewTitle = [NSString stringWithFormat:@"%@ %@",kBook_Title,location.spaLocation.locationName];
    [ResortSuiteAppDelegate changeNavigationBarTitleFormat:self text:viewTitle fontSize:nil];
...
-(void)displayBookingConfirmation
{
    ...
    {
        [staticLabelTexts insertObject:kStaffName_Title atIndex:2];
...
-(void)drawBookButton
{
	...
	//CGFloat bookButton_Y = ([UIManager isCurrentDeviceTypeIPhone5])?actionButtonYcord+40:actionButtonYcord;
...
{
    [self createTitleHeader:kSelectAppointment_Title yPosition:RSCBCTiltleYcord];
    [self createMessage:kTimeSlotNotAvailabel_Message yPostion:RSCBCMessageYcord];
...
-(NSDate *)dateFromString:(NSString *)stringDate {
    ...
    //[dateFormatter setLocale:[[[NSLocale alloc] initWithLocaleIdentifier:kEn_US_POSIX] autorelease]];
