...
-(void) generateAccountSummary
{
	...
	if (statementPeriod != nil) {
		[self displaySeperatorImage:label2Y_cord + ([membershipValue count] )*(LabelCord_TwoLinesDifference - LabelCord_height)+18];
	}
	else {
...
{
    float labelKeyWidth = ScrollLabel_Width-4;
	for (int titleIndex = 0; titleIndex < [tagsTitle count]; titleIndex++)
	{
		...
		if ([tagsTitle objectAtIndex:titleIndex] != nil)
		{
			...
			NSLog(@"ScrollLabel_Width&Label =%@",label1.text);
			if (titleIndex == 0) {
				label1.frame = CGRectMake(Label1Cord_x, label2Y_cord, labelKeyWidth, LabelCord_height);	//Label1Cord_y dont need to use it as now we are addin them to scrool view
			}
			else {
				label1.frame = CGRectMake(Label1Cord_x, label2Y_cord + titleIndex* (LabelCord_TwoLinesDifference - LabelCord_height), labelKeyWidth, LabelCord_height);
			}
		...
		else
		{
			...
			if (titleIndex == 0) {
				label1.frame = CGRectMake(Label1Cord_x, label2Y_cord, labelKeyWidth, LabelCord_height);
			}
			else {
				label1.frame = CGRectMake(Label1Cord_x, label2Y_cord + titleIndex*(LabelCord_TwoLinesDifference - LabelCord_height), labelKeyWidth, LabelCord_height);
			}
		...
		if (titleIndex < [tagsValue count] && [tagsValue objectAtIndex:titleIndex] != nil)
		{
			...
			if (titleIndex == 0) {
                ...
                label2Y_cord = (numOfLine)>1?label2Y_cord-3:label2Y_cord;
		...
		else
		{
			...
			if (titleIndex == 0) {
				label2.lineBreakMode = UILineBreakModeWordWrap;
				label2.numberOfLines = 0;
				label2.frame = CGRectMake(Label2Cord_x, label2Y_cord, ScrollLabel_Width+2, LabelCord_height*numOfLine);
				...
				label2.lineBreakMode = UILineBreakModeWordWrap;
				label2.numberOfLines = 0;
				label2.frame = CGRectMake(Label2Cord_x, label2Y_cord + titleIndex*(LabelCord_TwoLinesDifference - LabelCord_height), ScrollLabel_Width+2, LabelCord_height*numOfLine);
				[label2 setText:DataNotAvailable];
				//set the next y cord
				label2Y_cord = [self calculateNextY_CordFromPreviousY_cord:label2Y_cord andNumOfLine:numOfLine];
			}
