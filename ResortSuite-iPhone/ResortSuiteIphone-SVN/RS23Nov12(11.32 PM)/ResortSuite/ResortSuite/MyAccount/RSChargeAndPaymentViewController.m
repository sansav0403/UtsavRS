...
- (void)viewDidLoad {
    [super viewDidLoad];
	CGFloat screenHeight = [UIManager currentScreenHeight];
	UIImageView *imageViewBackgroud = [[UIImageView alloc] initWithFrame:CGRectMake(0,0, 320, screenHeight)];
	...
	UIImageView *imageViewBackgroudOverLay = [[UIImageView alloc] initWithFrame:CGRectMake(0,44, 320, screenHeight - 80)];
...
{
	float dynamic_Y_cord = 85;
	for (int labelIndex = 0; labelIndex < [self.constLabels count]; labelIndex++)
	{
        NSLog(@"%f",dynamic_Y_cord);
		UILabel *label2 = [[UILabel alloc] initWithFrame:CGRectZero];
		UILabel *label1 = [[UILabel alloc] initWithFrame:CGRectMake(Label1Cord_x, dynamic_Y_cord, LabelCord_width, LabelCord_height)]; //
        ...
        CGSize titleLabelSize = [[self.constLabels objectAtIndex:labelIndex] sizeWithFont:[UIFont boldSystemFontOfSize:FONT_SIZE] constrainedToSize:CGSizeMake(130.0, 40.0) lineBreakMode:UILineBreakModeWordWrap];
        
        label1.numberOfLines = 0;
        label1.frame = CGRectMake(Label1Cord_x, dynamic_Y_cord, LabelCord_width , titleLabelSize.height );
        //dynamic_Y_cord += titleLabelSize.height+7;// + Y_diff;//(Y_diff *labelIndex);
        NSLog(@"%f ,height=%f",dynamic_Y_cord, titleLabelSize.height);
		...
		int label2FrameX = Screen_Width/2 -11;
		label2.frame = CGRectMake(label2FrameX , dynamic_Y_cord, (Screen_Width - label2FrameX - 10) , LabelCord_height);
        ...
        CGSize valueLabelSize = [[self.dynamicLabels objectAtIndex:labelIndex] sizeWithFont:[UIFont systemFontOfSize:FONT_SIZE]
                                 
                                                                          constrainedToSize:CGSizeMake(140.0, 40.0) lineBreakMode:UILineBreakModeWordWrap];
        label2.frame = CGRectMake(label2FrameX, dynamic_Y_cord, valueLabelSize.width+10 , valueLabelSize.height );
        
		//}
        float maxLebelHeight = (valueLabelSize.height > titleLabelSize.height)?valueLabelSize.height:titleLabelSize.height;
        
        dynamic_Y_cord += maxLebelHeight+7;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell == nil) {
        ...
        //cell = [[RSChargeTableCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
    ...
    /*NSLog(@"[tableObjects objectAtIndex:indexPath.row]=%@",[tableObjects objectAtIndex:indexPath.row]);
    NSLog(@"objectLabels = %@",[[[tableObjects objectAtIndex:indexPath.row] objectLabels] objectAtIndex:0]);
    [cell.fieldLabel1 setTitle:[[[tableObjects objectAtIndex:indexPath.row] objectLabels] objectAtIndex:0]];	//  will give string
	[cell.fieldLabel2 setTitle:[[[tableObjects objectAtIndex:indexPath.row] objectLabels] objectAtIndex:1]];
	[cell.fieldLabel3 setTitle:[NSString stringWithFormat:@"$ %@",[[[tableObjects objectAtIndex:indexPath.row] objectLabels] objectAtIndex:2]]];*/
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath 
{
	if ([self.navigationItem.title isEqualToString:Club_Charges])
	{
		...
		if([postingItems count]>0)
		{
			NSString *viewTitle = [NSString stringWithFormat:@"%@: %d",kItemsInFolio_Title,[[postingItems objectAtIndex:0] ItemId]];      //on view title the view ll change for charge or payment
			...
			for (int postingItemsIndex = 0; postingItemsIndex < [postingItems count]; postingItemsIndex++) 
			{
				for (int itemsIndex = 0; itemsIndex < [[[postingItems objectAtIndex:postingItemsIndex] items] count]; itemsIndex++) 
				{
					...
					NSString *ObjectDescription = [NSString stringWithFormat:kYourExpences_Title];
