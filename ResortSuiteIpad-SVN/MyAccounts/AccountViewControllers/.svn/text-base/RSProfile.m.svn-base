...
-(id) initWithSelectedAccount:(Account *)account
{
    self = [super init];
    if (self) {
...
//	ResortSuiteAppDelegate *appDelegate = (ResortSuiteAppDelegate *)[[UIApplication sharedApplication] delegate];
    accountManager = [RSMyAccountManager sharedInstance];
	
	NSString *customerAdress = [NSString stringWithFormat:@"%@ %@ %@ %@ %@ %@.", 
								accountManager.modelClubAccounts.accCustomer.address.addressLine1,
								accountManager.modelClubAccounts.accCustomer.address.addressLine2,
								accountManager.modelClubAccounts.accCustomer.address.city,
								accountManager.modelClubAccounts.accCustomer.address.province,
								accountManager.modelClubAccounts.accCustomer.address.country,
								accountManager.modelClubAccounts.accCustomer.address.postalCode
								];
    
    NSArray *dynamicLblTxtArray = [[NSArray alloc]initWithObjects:
                                   [self.selectedAccount accountId],
                                   [self.selectedAccount classType],
                                   [self.selectedAccount statementCustomerName],
                                   
                                   accountManager.modelClubAccounts.accCustomer.customerCode,
                                   accountManager.modelClubAccounts.accCustomer.customerId,
                                   accountManager.modelClubAccounts.accCustomer.VIPLevel,
                                   
                                   customerAdress,
                                   accountManager.modelClubAccounts.accCustomer.phoneNumber,
                                   accountManager.modelClubAccounts.accCustomer.emailAddress,
                                   nil];
...
//	ResortSuiteAppDelegate *appDelegate = (ResortSuiteAppDelegate *)[[UIApplication sharedApplication] delegate];
	...
	for(int membershipIndex=0; membershipIndex< [[self.selectedAccount memberShips] count]; membershipIndex++)
	{
		label2Y_cord += [self.constLabelText count]*(LabelCord_TwoLinesDifference - LabelCord_height)-50;//sub -60 edit
		
		NSString *memberName = [NSString stringWithFormat:@"%@ %@ %@", [[[self.selectedAccount memberShips] objectAtIndex:membershipIndex] salutation],
								[[[self.selectedAccount memberShips] objectAtIndex:membershipIndex] firstName],
								[[[self.selectedAccount memberShips] objectAtIndex:membershipIndex] lastName] ];
		
		NSArray* membershipValue = [[NSArray alloc]initWithObjects:
									[[[self.selectedAccount memberShips] objectAtIndex:membershipIndex] name],
									memberName,
									[[[self.selectedAccount memberShips] objectAtIndex:membershipIndex] status],
									[[[self.selectedAccount memberShips] objectAtIndex:membershipIndex] effectiveDate],
									[[[self.selectedAccount memberShips] objectAtIndex:membershipIndex] expiryDate],
									nil];
...
//	ResortSuiteAppDelegate *appDelegate = (ResortSuiteAppDelegate *)[[UIApplication sharedApplication] delegate];
	...
	for(int memberIndex=0; memberIndex< [[self.selectedAccount members] count]; memberIndex++)
	{
		label2Y_cord += [self.membershipTags count]*(LabelCord_TwoLinesDifference - LabelCord_height) + 15 ;//sub -40 edit
		
		NSString *memberName = [NSString stringWithFormat:@"%@ %@ %@", [[[self.selectedAccount members] objectAtIndex:memberIndex] salutation],
								[[[self.selectedAccount members] objectAtIndex:memberIndex] firstName],
								[[[self.selectedAccount members] objectAtIndex:memberIndex] lastName] ];
		
		NSArray* accountCustomerValue = [[NSArray alloc]initWithObjects:
										 memberName,
										 [[[self.selectedAccount members] objectAtIndex:memberIndex] VIPLevel],
										 nil];
