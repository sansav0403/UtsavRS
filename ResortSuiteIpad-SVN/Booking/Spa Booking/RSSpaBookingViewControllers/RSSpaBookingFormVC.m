...
- (void)viewDidLoad
{
    ...
    [self.doneButton setTitle:kCheckAvailability_Title forState:UIControlStateNormal];
    ...
    switch (selectedSection)
    {
        ...
        case PrefStaffSection:			
            if ([[subFieldArray objectAtIndex:0] isEqualToString:NONE]) {
                ...
                if ([[subFieldArray objectAtIndex:3] isEqualToString:Gender_Male]) {
                    prefStaffGender = @"M";
                }
                else if ([[subFieldArray objectAtIndex:3] isEqualToString:Gender_Female]) {
                    prefStaffGender = @"F";
                }			
                else {
                    prefStaffGender = @"";
